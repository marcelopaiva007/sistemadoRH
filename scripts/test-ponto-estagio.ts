...
const r1 = apurarLimiteEstagio(comAlmoco, brt(QUA, "17:00")); // 14:00 BRT, 1h depois da volta
...
ok(
  apurarLimiteEstagio(soEntrou, brt(QUA, "14:00")).minutosHoje === 180,
  "entrou às 08:00 e são 11:00 → 3h, mesmo sem ter batido a saída",
);
ok(
  apurarLimiteEstagio(soEntrou, brt("2026-08-13", "14:00")).minutosHoje === 0,
...
const r2 = apurarLimiteEstagio(seisHoras, brt(QUA, "17:05"));
ok(r2.minutosHoje === 360 && r2.excedeuDia, "6h corridos excedem o teto diário");
ok(
  (avisoDeLimiteEstagio(r2) ?? "").includes("6h"),
  "o aviso diz quantas horas a pessoa tem, não um código",
);
ok(
  (avisoDeLimiteEstagio(r2) ?? "").includes("registrada"),
  "e diz que a marcação FOI registrada — avisa, não bloqueia",
);
ok(avisoDeLimiteEstagio(apurarLimiteEstagio(comAlmoco, brt(QUA, "17:00"))) === null, "dentro do teto, nenhum aviso");
...
const noSabado = apurarLimiteEstagio(seisDias, brt("2026-08-15", "16:05"));
...
const estourou = apurarLimiteEstagio(comMeiaHoraAMais, brt("2026-08-15", "17:30"));
ok(estourou.excedeuSemana, "30h30 excede o teto semanal");
ok((avisoDeLimiteEstagio(estourou) ?? "").includes("semana"), "e o aviso fala da semana");

// Domingo fecha a semana da segunda anterior; a segunda seguinte zera.
const domingo = apurarLimiteEstagio(seisDias, brt("2026-08-16", "14:00"));
ok(domingo.minutosSemana === 1800, "domingo 16/08 ainda soma a semana que começou em 10/08");
const segundaSeguinte = apurarLimiteEstagio(seisDias, brt("2026-08-17", "14:00"));
...
const r3 = apurarLimiteEstagio(noturno, brt("2026-08-13", "00:30")); // 21:30 BRT, ainda dia 12
...
ok(emHorasEMinutos(LIMITE_ESTAGIO_MIN_DIA) === "5h", "o teto diário se apresenta como 5h");
